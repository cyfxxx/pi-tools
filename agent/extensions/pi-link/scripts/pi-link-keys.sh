...
#
# 思路：所有设备的公钥集中存放在仓库 extensions/pi-link/keys/authorized_keys（git 同步），
...
EXT_DIR="$(dirname "$SCRIPT_DIR")"           # pi-link 扩展目录
PI_HOME="$(dirname "$(dirname "$EXT_DIR")")" # 仓库根（~/.pi）
KEYS_FILE="$EXT_DIR/keys/authorized_keys"    # 扩展内公钥合集
