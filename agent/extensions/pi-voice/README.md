...
- **唤醒词无反应（统计恒为 0）**（2026-08-31 自 skill 草稿迁入）：三诊断法——①验证基线：手动录音→转写，确认麦克风与 ASR 正常，基线不通先修基线；②单测唤醒链路：进监听说唤醒词，看统计计数与日志；③按三类故障面分流：采集通道丢数据 / 唤醒模型缺陷 / 回调触发时机错误。已确认坑：a) sherpa-onnx KWS 经 stdout pipe 上报在扩展沙箱下不可靠（事件静默丢失，最常见）→ 加 `--file-format=wav` 落盘 + 轮询读文件代替解析 stdout；b) 加载期调用 sendMessage 会使整个 pi 启动崩溃（runtime 未绑定、sendMessage 是抛错桩）→ 依赖 runtime 的 API 延迟到首次用户交互或显式开关，禁止模块顶层/load 回调同步调用；c) KWS 3.3M 拼音模型自身误检/漏检 → 排查代码前先换模型或调阈值交叉验证。注意：扩展源码改动不热载，验证必须重启 pi。
- **autoWake 后台监听实现要点**：配置项放 config.ts，逻辑在 core.ts；开关走扩展 slash 命令子命令（≤2 命令约定）；关闭时必须终止唤醒子进程并清理文件轮询定时器防僵尸进程。交付前验证：唤醒计数 >0、开关各一次、连续 10 次无误判漏判。