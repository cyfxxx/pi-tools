...
if [ -x "$PI_HOME/scripts/install/install-wrapper.sh" ]; then
  "$PI_HOME/scripts/install/install-wrapper.sh" --ensure --quiet >> "$LOG_DIR/wrapper-ensure.log" 2>&1 || \
...
try:
    ...
    sys.exit(0)  # 解析失败 → 不禁用（输出空串）
print('1' if d.get('enabled') is True else '')
" "$AUTOPILOT_CFG")
    if [ -z "$ENABLED" ]; then
      exit 0
    fi
