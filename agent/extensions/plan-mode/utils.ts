...
/**
 * Determine if a user message is requesting a plan revision vs. a normal question.
 */
export function isPlanRevisionIntent(text: string): boolean {
	const normalized = text.toLowerCase().replace(/\s+/g, " ").trim();
	if (!normalized) return false;

	// Short messages (<100 chars) without plan keywords are rarely revisions
	...
	// If it's purely a clarification question, treat as normal question
