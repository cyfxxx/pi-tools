...
- **`ask_user` 工具**：模型向用户提问并获取选择回答。支持自定义问题、选项标签和描述，在正常模式和计划模式均可使用。
  - **单选模式**：选择后显示确认对话框，支持"确认"或"修改"，防止误选
  - **多选模式**：已选选项显示 ✓ 标记，点击可取消单个选择，支持"完成选择"或"取消全部"
  - 参数结构：`question`（问题内容）、`header`（简短标签）、`options`（选项数组，至少2个）、`multiple`（是否允许多选）。工具返回用户选择的选项标签，可用于后续逻辑分支。
  - 与计划模式的三选一功能不冲突（三选一是 `agent_end` 事件触发的自动选择，ask_user 是工具调用的主动提问）。
