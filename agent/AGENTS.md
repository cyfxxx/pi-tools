...
- `agent/lib/` — 共享库：`context-budget.ts`（统一 token 预算/估算/裁剪/缓存统计）、`auto-compact.ts`（按窗口比例自动压缩阈值+防抖）、`usage-diag.ts`（每轮 LLM 用量诊断记录/汇总）、`note-store.ts`、`token-budget.ts`/`prune.ts`（兼容层）
...
单套件：`cd agent/extensions/<ext> && ./node_modules/.bin/vitest run`（pi-web-search 72 / pi-memory 53 / pi-autopilot 86 / pi-browser 23 / pi-context 18 / plan-mode 10 用例）
...
- **自动压缩**：pi 内置压缩阈值 = 窗口 − reserveTokens，对 1M 窗口模型高达 96.7 万形同虚设；由 pi-context 在 agent_end 按窗口比例触发（>256K 窗口 20% / ≤256K 85%），阈值计算与防抖见 `lib/auto-compact.ts`；ctx.compact() 会 abort 当前 agent，故只能在 agent_end 调用
