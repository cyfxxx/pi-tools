...
- **plan-mode subagent 开放**（参考 opencode explore 只读子代理）：⚠️ 已撤回——2026-08 实测 subagent 工具在 TUI 会话未进 registry（`Tool subagent not found`，白名单超前暴露致模型空转 2 轮）。`assertPlanSubagentAllowed`（utils.ts）与测试保留、注释标注"待注册稳定后启用"；当前 plan 白名单仅 read/bash/grep/glob
