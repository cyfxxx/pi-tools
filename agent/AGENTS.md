...
单套件：`cd agent/extensions/<ext> && ./node_modules/.bin/vitest run`（pi-web-search 75 / pi-memory 94 / pi-autopilot 106 / pi-browser 25 / pi-context 81 / plan-mode 69 / pi-tmux 19 / pi-voice 128 / pi-link 57 用例，2026-08-17 实测，以 test-all.sh 输出为准）
...
subagent 无 vitest：`cd agent/extensions/subagent && node --experimental-strip-types --import ./tests/loader.mjs ./tests/test.mjs`（63 用例）
