...
if (AS_JSON) {
  console.log(JSON.stringify({
    ...
    counts: { staleCandidates: stale.length, promotionCandidates: promote.length, conflictGroups: conflicts.length, junkSuspects: junk.length },
...
console.log(`有效条目 ${live.length}/${entries.length} | 规则: 淘汰(${STALE_DAYS}天冷+低置信+零复现) 升格(recurrence≥${PROMOTE_MIN} 的 solutions/fact，非垃圾) 冲突(标题重复) 垃圾(content 无实义/噪声标题)`)
console.log('')
