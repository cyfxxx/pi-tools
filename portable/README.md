...
3. 运行 `.\bin\setup.ps1`（自动：下载 Node LTS → npmmirror 装 pi → 下载 ffmpeg/PortableGit → 装扩展运行时依赖 → 拷入 ca-bundle/tmux shim）
...
| pi-browser 浏览器 | cloakbrowser 首次调用自动下载（cloakbrowser.dev → GitHub release 直连；`~/.cloakbrowser/chromium-<ver>/`，~700MB 解压） | stealth 定制 Chromium；下载被墙时可手动设 `CLOAKBROWSER_DOWNLOAD_URL`/`CLOAKBROWSER_BINARY_PATH` 覆盖 |
