...
- `extensions/` — 12 个扩展：subagent / pi-context / plan-mode / pi-autopilot / pi-memory / pi-web-search / pi-browser / pi-tmux / pi-voice / pi-link / pi-intervention / pi-mode
...
`patch-truncate-type.mjs`（truncateToWidth text.slice 类型守恒，修复非字符串值导致崩溃）
...
共 12 个 patch 文件由 rebuild.sh 自动执行（幂等）：11 个无条件（上列除 playwright-core 外全部）+ `patch-playwright-core.mjs` 仅 Termux 条件执行；pi update 升级 dist 后需重跑 rebuild.sh（或手动 node 执行对应脚本）。
